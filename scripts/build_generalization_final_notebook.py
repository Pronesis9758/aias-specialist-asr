...
def build() -> Path:
    ...
    notebook["cells"] = [
        ...
        nbf.v4.new_code_cell(
            ...
            "# Colab은 실행 중인 커널에서 새 editable-install .pth를 즉시 다시 읽지 않을 수 있습니다.\n"
            "# 저장소의 src 경로를 명시해 설치 직후에도 현재 커널과 하위 CLI가 같은 코드를 사용하게 합니다.\n"
            "PROJECT_SRC = str(Path(PROJECT_DIR) / 'src')\n"
            "if PROJECT_SRC not in sys.path:\n"
            "    sys.path.insert(0, PROJECT_SRC)\n"
            "import importlib\n"
            "importlib.invalidate_caches()\n"
            "import aias_specialist\n"
            "print('AIAS package:', Path(aias_specialist.__file__).resolve())\n\n"
            "def run_aias(*args):\n"
